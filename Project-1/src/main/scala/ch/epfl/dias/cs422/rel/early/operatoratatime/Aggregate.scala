...
class Aggregate protected (
  ...
  protected var stored: IndexedSeq[(Any, Column)] = IndexedSeq[(Any, Column)]()
  protected var group_map : Map[Any,Any] = _
  protected var group_key : IndexedSeq[Any] = _
  protected var result : IndexedSeq[Column] = IndexedSeq[Column]()
  ...
  override def execute(): IndexedSeq[Column] =  {
    table = input.execute()
    if(table.isEmpty){table_tmp=table}
    else {
      table_tmp=Prepare_table(table)
    }
    ...
    if(aggCalls.length!=0)
    {
      ...
      for (x<- aggCalls)
      {
        group_map = stored.groupMapReduce(t => t._1)(element => x.getArgument(element._2))((e1, e2) => x.reduce(e1, e2))
    ...
    {
      mode=true
      group_map = stored.groupBy(item => item._1)
      group_key = group_map.keys.toIndexedSeq
      construct_result(group_map,groupSet,group_key,mode)
    }
